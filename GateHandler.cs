...
public class GateHandler : GameModeController
{
    public List<JsonGateData> listGateData;
    public ScrollRect scrollRect;
    public GameObject objChapterCell_Down;
    public GameObject objChapterCell_Up;

    //public QuestionHandler questionHandler; 進入對戰地圖要用的資訊

    public List<GateCellHandler> listCell = new List<GateCellHandler>();

    public int[] ai_gate;    //要改
    ...
    public override void OnNavigationStart()
    {
        ...
        foreach (var id in ai_gate)   //要改
        {
            listGateData.Add(JsonDataManager.Singleton.dictJsonGateData[id]);
        }

        AddGateContent();
    }

    private void AddGateContent()
    {
        /*
        ...
        */
        ...
        for (int i = 0; i < listGateData.Count; i++)
        {
            if (i % 2 == 0)
            {
                GameObject cell = UnityTool.AddUGUIChild(scrollRect.content, objChapterCell_Down);
                GateCellHandler handler = cell.GetComponent<GateCellHandler>();
                handler.SetCellContent(listGateData[i], OnCellClickCallback);
                listCell.Add(handler);
            }
            else
            {
    ...
    private void OnCellClickCallback(JsonGateData aData)
    {
